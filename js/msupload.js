...
function createUpload(wikiEditor){
		...
		if ( $( '#sf_free_text' ).length == 1 ) {
			element = $( '#sf_free_text' ).closest( '.wikiEditor-ui' );
		} else if ( $.inArray( mw.config.get( 'wgAction' ), ['edit', 'submit'] ) !== -1 ) {
			element = $( '.wikiEditor-ui' );
		} else {
			return;
		}

		if ( element.find( 'upload_container' ).length > 0 ) { // return if already binded
			mw.log( 'msupload already binded' );
			return;
		}
		...
		if(wikiEditor==true){		
			...
			var upload_tab = $(document.createElement("div")).attr('class','group ').appendTo( element.find( '#wikiEditor-ui-toolbar .toolbar.section.section-main' ) );
 			upload_container.appendTo(upload_tab);
			//create upload div  
			var upload_div = $(document.createElement("div")).attr("id","upload_div").insertAfter( element.find( '#wikiEditor-ui-toolbar' )); 
			element.find('#wikiEditor-ui-toolbar .tool .options').css('z-index', '2'); //headline dropdown		
		...
		try{
    		...
    		$(document.createElement("a")).text(mw.msg('msu-insert_link')).click(function(e) { //click
  			    if(msu_vars.use_mslinks == 'true'){
  			    	msu_vorlage_insert('{{#l:'+file.name+'}}','','', upload_container); // insert link		
  			    } else {
  			    	msu_vorlage_insert('[[:File:'+file.name+']]','','', upload_container); // insert link
  			    }
            ...
            if (file.group == "pic"){
        		...
        		gallery_arr.push(file.name);	
        		  ...
        		  if(gallery_arr.length== 2){ //only at first time add click function
	        		  		gallery_insert.click(function(e) { //click
	  							...
	  							add_gallery( upload_container ); //to take always the actual list
        		...
        		$(document.createElement("a")).text(mw.msg('msu-insert_picture')).click(function(e) { //click
        			...
        			msu_vorlage_insert('[[File:'+file.name+msu_vars.imgParams+']]','','', upload_container);
...
function add_gallery( upload_container ){
	gallery_text = "Image:";
